...
class TaxiBot:
    def __init__(self, bot_data=os.path.join(dir_path, 'resources/data.json')):
    ...
    def update_order_info(self, user_mess, overwrite=False):
        for key in self.requires["orders"]:
            for rkey in self.requires[key]["match_regex"]:
                founds = re.findall(rkey, user_mess, re.IGNORECASE)
                if len(founds) > 0:
                    if key not in self.order_data or overwrite:
                        if key == '{para.amount}' or key == '{para.type}':
